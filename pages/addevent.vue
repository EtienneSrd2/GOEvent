<template>
  <div id="createArticlePage">
    <header class="px-3 py-2 d-flex justify-content-center">
      <h3 class="text-uppercase pt-3">Ajouter un évènement</h3>
    </header>
    <div class="form-contener pb-5">
      <b-input-group id="input-group-1" label="Lien url" label-for="input-3" class="label mt-3">
        <b-form-input
          ...
          required
          label="Photo"
          placeholder="URL de votre évènement"
      ...
      <template v-if="previewLoaded">
        ...
        >Valider votre évènement</b-button>
      </template>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      errorMessage: '',
      categories: ['ecoresponsable', 'conference', 'festival', 'sport'],
      previewLoaded: false
    }
  },
